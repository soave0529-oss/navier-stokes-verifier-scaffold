...
NON_CLAIMS = (
    "read_only_smooth_continuation_source_read_packet_dependency_guard",
    "canonical_report_freshness_only",
    "non_promotional_gap_004_dependency_audit",
...
class SmoothContinuationSourceReadPacketDependencyCheck:
    key: str
    expected: str
    observed: str
    passed: bool
...
@dataclass(frozen=True)
class Lemma0252SmoothContinuationSourceReadPacketDependency:
    schema_version: int
    lemma_id: str
    ...
    passed_smooth_continuation_source_read_packet_dependency_check_count: int
    ...
    issues: tuple[str, ...]
    checks: tuple[SmoothContinuationSourceReadPacketDependencyCheck, ...]
...
def _check(
    *,
    ...
    observed: object,
    source_artifacts: tuple[str, ...],
) -> SmoothContinuationSourceReadPacketDependencyCheck:
    return SmoothContinuationSourceReadPacketDependencyCheck(
        key=key,
...
def build_smooth_continuation_source_read_packet_dependency(
    *,
...
) -> Lemma0252SmoothContinuationSourceReadPacketDependency:
    ...
    checks = (
        ...
        _check(
            key="checklist.source_blocker_id.expected_smooth_continuation_bridge",
            expected=SOURCE_BRANCH,
            observed=checklist.get("source_blocker_id"),
            source_artifacts=checklist_source,
        ),
        _check(
            key="queue.gap_004.source_branch.matches_packet",
    ...
    return Lemma0252SmoothContinuationSourceReadPacketDependency(
        schema_version=1,
        ...
        smooth_continuation_source_read_packet_dependency_check_count=len(checks),
        passed_smooth_continuation_source_read_packet_dependency_check_count=sum(
            1 for check in checks if check.passed
        ),
        failed_smooth_continuation_source_read_packet_dependency_check_count=len(
            issues
        ),
        smooth_continuation_source_read_packet_dependency_consistent=len(issues) == 0,
        ...
        missing_required_input_count=int(checklist.get("missing_required_input_count", 0)),
        criterion_input_missing_branch_count=int(
            checklist.get("criterion_input_missing_branch_count", 0)
        ),
        extra_assumption_branch_count=int(
            checklist.get("extra_assumption_branch_count", 0)
        ),
...
def render_markdown(
    report: Lemma0252SmoothContinuationSourceReadPacketDependency,
) -> str:
    lines = [
        "# Lemma 0252 Smooth-Continuation Source Read Packet Dependency",
        "",
        "## Summary",
        "",
...
def check_sources(
    ...
    return True, "all smooth-continuation source-read dependency sources exist"


def check_consistent(
    report: Lemma0252SmoothContinuationSourceReadPacketDependency,
) -> tuple[bool, str]:
    if not report.smooth_continuation_source_read_packet_dependency_consistent:
        return (
            False,
            "inconsistent smooth-continuation source-read packet dependency: "
            + ", ".join(report.issues),
        )
    return True, "smooth-continuation source-read packet dependency is consistent"
...
def check_blocked(
    ...
    if (
        ...
        return False, "smooth-continuation source-read packet dependency is not blocked"
    return True, "smooth-continuation source-read packet dependency remains blocked"
...
def check_output(
    ...
    if not output.exists():
        return False, f"missing smooth-continuation source-read dependency output: {output}"
    observed = output.read_text(encoding="utf-8")
    if observed != expected:
        return False, f"stale smooth-continuation source-read dependency output: {output}"
    return True, f"fresh smooth-continuation source-read dependency output: {output}"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Render a read-only dependency guard for the lemma_0252 smooth-continuation "
            "source-read packet."
        )
    )
    ...
    report = build_smooth_continuation_source_read_packet_dependency()
    output = args.output
    if output is None:
