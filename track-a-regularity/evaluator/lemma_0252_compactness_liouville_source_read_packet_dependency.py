...
from lemma_0252_compactness_liouville_source_read_packet import (
...
NON_CLAIMS = (
    "read_only_compactness_liouville_source_read_packet_dependency_guard",
    "canonical_report_freshness_only",
    "non_promotional_gap_003_dependency_audit",
...
class CompactnessLiouvilleSourceReadPacketDependencyCheck:
    key: str
    expected: str
    observed: str
    passed: bool
...
@dataclass(frozen=True)
class Lemma0252CompactnessLiouvilleSourceReadPacketDependency:
    schema_version: int
    lemma_id: str
    ...
    compactness_liouville_source_read_packet_dependency_check_count: int
    passed_compactness_liouville_source_read_packet_dependency_check_count: int
    failed_compactness_liouville_source_read_packet_dependency_check_count: int
    compactness_liouville_source_read_packet_dependency_consistent: bool
    packet_status: str
    ...
    issues: tuple[str, ...]
    checks: tuple[CompactnessLiouvilleSourceReadPacketDependencyCheck, ...]
...
def _check(
    *,
    ...
    observed: object,
    source_artifacts: tuple[str, ...],
) -> CompactnessLiouvilleSourceReadPacketDependencyCheck:
    return CompactnessLiouvilleSourceReadPacketDependencyCheck(
        key=key,
...
def build_compactness_liouville_source_read_packet_dependency(
    *,
...
) -> Lemma0252CompactnessLiouvilleSourceReadPacketDependency:
    ...
    checks = (
        ...
        _check(
            key="packet.gap_id.expected_gap_003",
            expected=GAP_ID,
            observed=packet.get("gap_id"),
            source_artifacts=packet_source,
        ),
        _check(
            key="packet.source_branch.expected_compactness_liouville",
            expected=SOURCE_BRANCH,
            observed=packet.get("source_branch"),
            source_artifacts=packet_source,
        ),
        _check(
            key="checklist.source_blocker_id.expected_compactness_liouville",
            expected=SOURCE_BRANCH,
            observed=checklist.get("source_blocker_id"),
            source_artifacts=checklist_source,
        ),
        _check(
            key="queue.gap_003.source_branch.matches_packet",
        ...
        _check(
            key="checklist.checklist_item_count.expected_six",
            expected=6,
            observed=checklist.get("checklist_item_count"),
            source_artifacts=checklist_source,
        ),
        _check(
            key="checklist.theorem_branch_count.expected_three",
            expected=3,
        ...
        _check(
            key="source_refs.include_knss_2009",
            expected=True,
            observed=(
    ...
    return Lemma0252CompactnessLiouvilleSourceReadPacketDependency(
        schema_version=1,
        ...
        compactness_liouville_source_read_packet_dependency_check_count=len(checks),
        passed_compactness_liouville_source_read_packet_dependency_check_count=sum(
            1 for check in checks if check.passed
        ),
        failed_compactness_liouville_source_read_packet_dependency_check_count=len(
            issues
        ),
        compactness_liouville_source_read_packet_dependency_consistent=len(issues) == 0,
        ...
        dependent_on_new_result_count=int(
            checklist.get("dependent_on_new_result_count", 0)
        ),
        outside_setting_branch_count=int(checklist.get("outside_setting_branch_count", 0)),
...
def render_markdown(
    report: Lemma0252CompactnessLiouvilleSourceReadPacketDependency,
) -> str:
    lines = [
        "# Lemma 0252 Compactness Liouville Source Read Packet Dependency",
        "",
        "## Summary",
        "",
        ...
        (
            "- compactness_liouville_source_read_packet_dependency_check_count: "
            f"`{report.compactness_liouville_source_read_packet_dependency_check_count}`"
        ),
        (
            "- failed_compactness_liouville_source_read_packet_dependency_check_count: "
            f"`{report.failed_compactness_liouville_source_read_packet_dependency_check_count}`"
        ),
        (
            "- compactness_liouville_source_read_packet_dependency_consistent: "
            f"`{str(report.compactness_liouville_source_read_packet_dependency_consistent).lower()}`"
        ),
...
def check_sources(
    ...
    return True, "all compactness/Liouville source-read dependency sources exist"


def check_consistent(
    report: Lemma0252CompactnessLiouvilleSourceReadPacketDependency,
) -> tuple[bool, str]:
    if not report.compactness_liouville_source_read_packet_dependency_consistent:
        return (
            False,
            "inconsistent compactness/Liouville source-read packet dependency: "
            + ", ".join(report.issues),
        )
    return True, "compactness/Liouville source-read packet dependency is consistent"
...
def check_blocked(
    ...
    if (
        ...
        return False, "compactness/Liouville source-read packet dependency is not blocked"
    return True, "compactness/Liouville source-read packet dependency remains blocked"
...
def check_output(
    ...
    if not output.exists():
        return False, f"missing compactness/Liouville source-read dependency output: {output}"
    observed = output.read_text(encoding="utf-8")
    if observed != expected:
        return False, f"stale compactness/Liouville source-read dependency output: {output}"
    return True, f"fresh compactness/Liouville source-read dependency output: {output}"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Render a read-only dependency guard for the lemma_0252 compactness/Liouville "
            "source-read packet."
        )
    )
    ...
    report = build_compactness_liouville_source_read_packet_dependency()
    output = args.output
    if output is None:
