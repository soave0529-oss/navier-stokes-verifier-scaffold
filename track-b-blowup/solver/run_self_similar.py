"""Run bounded self-similar-inspired Track B diagnostics.

These are cheap localized-swirl probes. They are not exact self-similar
Navier-Stokes solutions and are not blow-up evidence.
"""
...
from initial_conditions import self_similar_swirl_initial
from pseudospectral import PseudoSpectralNSE, SolverConfig, write_csv


ROOT = THIS_DIR.parents[1]
PROFILES = {
    "wide": {"amplitude": 1.0, "width": 0.70, "axial_stretch": 0.75},
    "concentrated": {"amplitude": 1.8, "width": 0.42, "axial_stretch": 1.4},
}
...
def run_one(profile: str, out_dir: Path, cfg: SolverConfig, sample_every: int) -> dict[str, object]:
    ...
    u0 = self_similar_swirl_initial(cfg.n, **params)
    ...
    (out_dir / "config.json").write_text(json.dumps({**asdict(cfg), "sample_every": sample_every, **params}, indent=2, sort_keys=True) + "\n")
...
def _write_readme(out_dir: Path, report: dict[str, object], plots: list[Path]) -> None:
    ...
    lines = [
        f"# Self-Similar-Inspired Profile: {report['profile']}",
        "",
        "Cheap localized-swirl diagnostic. This is not an exact self-similar",
        "Navier-Stokes solution and is not evidence of blow-up.",
        "",
        "## Config",
        "",
        f"- N: `{report['n']}`",
        ...
        f"- profile params: `{report['profile_params']}`",
        "",
        "## Diagnostics",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--profile", choices=["wide", "concentrated", "all"], default="all")
...
def main() -> None:
    ...
    profiles = ("wide", "concentrated") if args.profile == "all" else (args.profile,)
    reports = []
    for profile in profiles:
        out_dir = args.out_root / f"self_similar_{profile}_{args.suffix}"
        reports.append(run_one(profile, out_dir, cfg, args.sample_every))
